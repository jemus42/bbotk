...
#'
#' @description
#' `OptimizerLocalSearch` class that implements a simple Local Search.  Local Search starts by
#' determining the `mu` initial points present in the [Archive] of the [OptimInstance].  If fewer
#' points than `mu` are present, additional points sampled uniformly at random are evaluated.
#'
#' In each iteration, for each of the `mu` initial points, `n_points` neighbors are generated by
#' local mutation.  Local mutation samples a single parameter that is to be mutated and then
#' proceeds as follows: Numeric parameters ([paradox::ParamDbl]) are mutated via Gaussian mutation
#' (using `sigma` scaled by the range of the parameter as a standard deviation).
#'
...
OptimizerLocalSearch = R6Class("OptimizerLocalSearch",
  ...
  public = list(
    ...
    initialize = function() {
      param_set = ps(
        ...
        n_points = p_int(lower = 1L, default = 1000L, tags = "required"),
        sigma = p_dbl(lower = 0L, default = 0.05, tags = "required")
      )
      param_set$values = list(mu = 10L, n_points = 1000L, sigma = 0.05)
  ...
  private = list(
    .optimize = function(inst) {
      ...
      points = inst$archive$data[, inst$archive$cols_x, with = FALSE]
      ...
      repeat {  # iterate until we have an exception from eval_batch
        neighbors = map_dtr(seq_len(nrow(points)), function(i) {
          map_dtr(seq_len(n_points), function(j) {
            mutate_point(points[i, ],
                         search_space = inst$search_space,
                         ranges = ranges,
                         ids_to_mutate = ids_to_mutate,
                         sigma = sigma)
          })
        })
...
mutate_point = function(point, search_space, ranges, ids_to_mutate, sigma = 0.05) {
  ...
  neighbor[1L, ][[id]] = mutate(neighbor[1L, ][[id]],
                                param = search_space$params[[id]],
                                range = ranges[[id]], sigma = sigma)
  neighbor
}
